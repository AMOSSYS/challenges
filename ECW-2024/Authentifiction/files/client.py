...
class ClientAuth:
    ...
    def auth_proof_request(self, salt: str, server_proof: bytes, server_challenge: bytes):
        ...
        if proof_decrypted[:CHALLENGE_LENGTH] != self.client_challenge:
            return {"error": "server proof has an incorrect client challenge or wrong password"}

        if proof_decrypted[-6:] != b"server":
            return {"error": "server proof does not terminate with 'server'"}

        # During the reverse engineering of the protocol, it appears that
        # the user name is not verified by the server: only the proof is checked,
        # and also the last 6 bytes (after decryption) to correspond to "client".
